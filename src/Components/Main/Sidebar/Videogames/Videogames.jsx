...
const VideogamesComponent = () => {
  ...
  // Sync clickedButtons state with the cart context on mount
  useEffect(() => {
    const clickedState = {};
    cart.forEach((item) => {
  ...
  // Filter products based on Genreid
  const filteredProducts = product.filter(
    (item) => item.Genreid === "GamingController"
  );
        ...
        {currentItems.map((item, index) => (
          ...
          >
            <Link
              ...
              className="block"
            >
              <div className="transition duration-300 ease-in-out hover:bg-gray-100">
                ...
                <div className="p-4">
                  <h3 className="mb-2 text-base font-medium text-gray-800 hover:text-blue-500">
                    {item.title}
                  </h3>
            ...
            <div className="py-4 border-t">
              <ul className="pl-2 ml-5 text-sm list-disc">
                <li>{item.size}</li>
